...
export default function EventsEdit({ event }: EventsEditProps) {
    ...
    const submit = (formEvent: FormEvent) => {
        ...
        post(event ? route('admin.events.update', event.uuid) : route('admin.events.store'), { forceFormData: true });
    };
        ...
        <AdminLayout
            header={
                <div>
                    ...
                    <h1 className="mt-2 text-3xl font-display font-semibold text-[#14234a]">{event ? 'Edit event' : 'New event'}</h1>
                </div>
            }
        >
            ...
            <form onSubmit={submit} className="grid max-w-4xl gap-5">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                    <label className="grid gap-2 text-sm font-bold text-[#14234a]">
                        Status
                        <select value={data.status} onChange={(formEvent) => setData('status', formEvent.target.value as 'draft' | 'published')} className="w-fit border-[#d7c8a9]">
                    ...
                    <label className="grid gap-2 text-sm font-bold text-[#14234a]">
                        Audience
                        <select value={data.audience} onChange={(formEvent) => setData('audience', formEvent.target.value as 'public' | 'members')} className="w-fit border-[#d7c8a9]">
                            <option value="public">Public — shown on the live website</option>
                            <option value="members">Members only — shown on the member dashboard only</option>
                        </select>
