...
export default function NewsEdit({ article }: NewsEditProps) {
    ...
    const submit = (event: FormEvent) => {
        ...
        post(article ? route('admin.news.update', article.uuid) : route('admin.news.store'), { forceFormData: true });
    };
        ...
        <AdminLayout
            header={
                <div>
                    ...
                    <h1 className="mt-2 text-3xl font-display font-semibold text-[#14234a]">{article ? 'Edit article' : 'New article'}</h1>
                </div>
            }
        >
