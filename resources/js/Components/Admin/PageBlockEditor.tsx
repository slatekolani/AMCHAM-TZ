...
function StatsEditor({ data, onChange }: EditorProps<Extract<PageBlock, { type: 'stats' }>['data']>) {
    ...
    const remove = (index: number) => onChange({ items: data.items.filter((_, i) => i !== index) });
    const add = () => onChange({ items: [...data.items, { label: '', value: '' }] });
...
function ValuesGridEditor({ data, onChange }: EditorProps<Extract<PageBlock, { type: 'values_grid' }>['data']>) {
    ...
    const remove = (index: number) => onChange({ items: data.items.filter((_, i) => i !== index) });
    const add = () => onChange({ items: [...data.items, { title: '', body: '' }] });
        ...
        <div className="grid gap-3">
            {data.items.map((item, index) => (
                <div key={index} className="grid gap-2 border border-[#eadfc8] bg-[#fbf8f0] p-3">
                    ...
                    {(item.image_upload || item.image) && (
                        <img src={item.image_upload ? URL.createObjectURL(item.image_upload) : item.image} alt="Card preview" className="h-36 w-full rounded-lg object-cover" />
                    )}
                </div>
            ))}
            <button type="button" onClick={add} className="w-fit border border-[#14234a] px-3 py-1.5 text-xs font-black uppercase tracking-[0.08em] text-[#14234a]">
...
function TagListEditor({ data, onChange }: EditorProps<Extract<PageBlock, { type: 'tag_list' }>['data']>) {
    const update = (index: number, value: string) => {
        onChange({ ...data, items: data.items.map((item, i) => (i === index ? value : item)) });
    };
    const remove = (index: number) => onChange({ ...data, items: data.items.filter((_, i) => i !== index) });
...
export function BlockEditorCard({ block, onChange }: { block: PageBlock; onChange: (block: PageBlock) => void }) {
    switch (block.type) {
        case 'hero':
            return <HeroEditor data={block.data} onChange={(data) => onChange({ ...block, data })} />;
        case 'stats':
            return <StatsEditor data={block.data} onChange={(data) => onChange({ ...block, data })} />;
        case 'heading_text':
            return <HeadingTextEditor data={block.data} onChange={(data) => onChange({ ...block, data })} />;
        case 'values_grid':
            return <ValuesGridEditor data={block.data} onChange={(data) => onChange({ ...block, data })} />;
        case 'tag_list':
            return <TagListEditor data={block.data} onChange={(data) => onChange({ ...block, data })} />;
        case 'fact':
            return <FactEditor data={block.data} onChange={(data) => onChange({ ...block, data })} />;
    }
}
